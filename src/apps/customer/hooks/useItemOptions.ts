...
function isNoneVariantOption(name: string): boolean {
  return /^ohne\b/i.test(name.trim())
}
...
function choicesFromCartItem(
  ...
  for (const group of variantGroups) {
    const ids = cartItem.variants
      .filter((v) => group.options.some((o) => o.id === v.id))
      .map((v) => v.id)
    if (ids.length) variantChoices[group.id] = ids
  }
...
export function useItemOptions(
  ...
  const setSingleVariant = (groupId: string, optionId: string) => {
    setVariantChoices((prev) => ({ ...prev, [groupId]: [optionId] }))
  }

  const toggleVariant = (group: ItemOptionGroup, optionId: string) => {
    setVariantChoices((prev) => {
      ...
      if (option && isNoneVariantOption(option.name)) {
        return { ...prev, [group.id]: [optionId] }
      }

      const withoutNone = current.filter((id) => {
        const selected = group.options.find((o) => o.id === id)
        return !selected || !isNoneVariantOption(selected.name)
      })

      if (group.maxSelect > 0 && withoutNone.length >= group.maxSelect) {
        return prev
      }

      return { ...prev, [group.id]: [...withoutNone, optionId] }
    })
  }
  ...
  const validate = () => {
    for (const group of variantGroups) {
      const count = (variantChoices[group.id] ?? []).length
